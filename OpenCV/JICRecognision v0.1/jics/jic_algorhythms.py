...
def proces2(image, frame):
    # Frame Detect
    frame, points = frame_detect(frame)
    if(points == None):
        return image, frame
    ...
    # Draw
    image = draw_frame(image, points)
    image = draw_frame(image, inner_points)
    image = draw_size(image, width, height)

    # Return result
    return image, frame

# Detect frame
def frame_detect2(image):
    mask = image[:, :, 3]
    contours, _ = cv.findContours(mask, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)

    if(contours[0] == 0 or contours[1] == 0):
        return image, None
...
def calculate_rectangle_sizes2(points):
