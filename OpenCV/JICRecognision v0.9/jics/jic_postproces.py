...
def proces(image, canvas):
    ...
    result = proces_data(original_canvas, line_points_x, line_points_y)

    # Return result
    return image, canvas, result
...
def draw_line(image, fixed_point, angle, length, color=(0, 0, 255)):
    ...
    cv.line(image, fixed_point, (x2, y2), color, 1)
...
def proces_data(image, line_points_x, line_points_y):
    ...
    width, height = convert_to_world(width, height) #cm
